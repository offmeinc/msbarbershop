...
export function PushPrompt({ userId, userRole }: PushPromptProps) {
  ...
  useEffect(() => {
    ...
    // 4. Check dismissal timeout (24 hours persistence)
    ...
    // On Android/Desktop, we can prompt even in browser, but PWA is preferred.
    const shouldShowForPlatform = isStandalone || (isAndroid && 'serviceWorker' in navigator && 'PushManager' in window);

    if (shouldShowForPlatform && !isRecentlyDismissed) {
          ...
          <div className="relative flex flex-col gap-4">
            ...
            <div className="space-y-1">
               <h3 className="text-sm font-black text-white flex items-center gap-2">
                 {getNotificationPermissionState() === 'denied' ? 'Ativar nas Configurações' : 'Não perca seus horários!'} <Zap className="w-3 h-3 text-amber-500" />
               </h3>
               <p className="text-[11px] text-neutral-400 font-medium leading-relaxed">
                 {getNotificationPermissionState() === 'denied'
                    ...
                    : 'Habilite as notificações push para receber alertas de agendamentos e atualizações importantes em segundo plano, mesmo com o app fechado.'
                 }
               </p>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1">
               ...
               >
                 ...
                 {getNotificationPermissionState() === 'denied' ? 'VER COMO ATIVAR' : 'ATIVAR NOTIFICAÇÕES AGORA'}
               </button>
               <p className="text-[9px] text-neutral-500 text-center font-bold uppercase tracking-tight">
                 Você pode desativar depois quando quiser
               </p>
            </div>
          </div>
        </div>
